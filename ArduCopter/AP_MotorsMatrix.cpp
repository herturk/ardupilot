...
void AP_MotorsMatrix::output_min()
{
    ...
    for( i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++ ) {
        if( motor_enabled[i] ) {
            ...
            // Our i2c esc works with 8 bit value so we map rc servo data
            motor_out[i] = map(motor_out[i], out_min, out_max, 0, 255);
            // Send motor value with I2C Driver 
           // hal.i2c->begin();
            hal.i2c->write(_motor_to_channel_map[i], motor_out[i]);
           // hal.i2c->end();
    ...
    for( i=min_order; i<=max_order; i++ ) {
        for( j=0; j<AP_MOTORS_MAX_NUM_MOTORS; j++ ) {
            if( motor_enabled[j] && _test_order[j] == i ) {
                ...
                // Our i2c esc works with 8 bit value so we map rc servo data
                test_out = map(_rc_throttle->radio_min + _min_throttle, out_min, out_max, 0, 255);
                // Send motor value with I2C Driver 
               // hal.i2c->begin();
                hal.i2c->write(_motor_to_channel_map[i], test_out);
              //  hal.i2c->end();
                //   hal.rcout->write(_motor_to_channel_map[j], _rc_throttle->radio_min + _min_throttle);
                hal.scheduler->delay(300);
                
                 // Our i2c esc works with 8 bit value so we map rc servo data
                test_out = map(_rc_throttle->radio_min, out_min, out_max, 0, 255);
                // Send motor value with I2C Driver 
              //  hal.i2c->begin();
                hal.i2c->write(_motor_to_channel_map[j], test_out);
             //   hal.i2c->end();
